...
def quickSelect(array, i, j, k):
    ...
    less, equal, greater = partition(array, i, j, array[randIndex])
    if k <= len(less) - 1:
        return quickSelect(less, 0, len(less) - 1, k)
    elif k >= len(less) + len(equal):
        return quickSelect(greater, 0, len(greater) - 1, k - len(less) - len(equal))
    else:
        return pivot
    # if i + k < left:
...
def solveProblem():
    ...
    for _ in range(queriesNumber):
        ...
        leftIndex = i - 1
        rightIndex = j - 1
        result = quickSelect(clones, leftIndex, rightIndex, k - 1)
        print(result)
