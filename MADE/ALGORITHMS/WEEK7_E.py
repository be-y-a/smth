...
class Solver:
    ...
    def solveForQueriesFast(self, n, queries):
    ...
    def solve(self):
        with open ('rmq.in') as infile:
            read_data = infile.read().split("\n")
        n, m = map(int, read_data[0].split())
        queries = []

        for i in range(m):
            i, j, q = map(int, read_data[i + 1].split())
        ...
        with open('rmq.out', 'w') as outfile:
            outfile.write(out)
...
#             q = answer(array, i, j)
#             q = random.randint(-2**31, 2**31)
#             allQueries.append((i, j, q))
#     return allQueries
