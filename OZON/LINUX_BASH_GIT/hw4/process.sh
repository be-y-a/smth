...
mvvm_filtered_rating_file_name='/tmp/mvvm_rating_filtered.csv'
gnu_plot_commands='/tmp/gnu_plot_commands'
...
cat hotels.csv | LC_ALL=C awk -F, '{ if ($18 >= 0) { print } }' > $mvvm_filtered_rating_file_name
{
  ...
  echo 'f(x) = m*x+b'
  echo "fit f(x) $mvvm_filtered_rating_file_name using 12:18 via m,b"
  echo "plot $mvvm_filtered_rating_file_name using 12:18 title 'cleanliness vs overall_ratingsource' with points, f(x) title 'fit'"
} >> gnu_plot_commands

gnuplot gnu_plot_commands
